...
export default function OccupancyRemovalsLog({ refreshToken = 0 }: Props) {
  ...
  return (
    <section className="card-glass mt-6 overflow-hidden">
      <div className="border-b border-surface-border/60 px-6 py-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h2 className="text-base font-semibold text-white">{t("occupancy.removals.title")}</h2>
            <p className="mt-1 text-sm text-slate-400">{t("occupancy.removals.subtitle")}</p>
          </div>
        </div>
      </div>

      {loading ? (
        <p className="px-6 py-8 text-center text-sm text-slate-500">{t("common.loading")}</p>
      ) : error ? (
        <p className="px-6 py-8 text-center text-sm text-rose-400">{error}</p>
      ) : events.length === 0 ? (
        <p className="px-6 py-8 text-center text-sm text-slate-500">{t("occupancy.removals.empty")}</p>
      ) : (
            ...
            <tbody>
              {events.map((event) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 font-medium text-slate-200">{event.zone ?? "—"}</td>
                  <td className="px-4 py-3 font-medium text-accent">{fmtMoney(event.price)}</td>
                  ...
                  <td className="max-w-md px-6 py-3">
                    <p className="truncate text-slate-300">{event.address ?? "—"}</p>
                    <p className="mt-0.5 truncate text-xs text-slate-600">{event.id}</p>
                  </td>
                </tr>
              ))}
            </tbody>
