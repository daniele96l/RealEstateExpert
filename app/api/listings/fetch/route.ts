...
import { fetchListingsScraped } from "@/lib/server/listings-fetch";
...
export async function POST(request: Request) {
  try {
    ...
    const data = await fetchListingsScraped(body.city, body.operation, portal, maxPages);
    const enriched = await enrichCityListingsCache({ ...data, provider: "direct" });
    await saveCache(enriched, market);
    return NextResponse.json(enriched);
  } catch (err) {
