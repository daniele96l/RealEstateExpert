...
import { computePurchaseCostsCz } from "./countries/cz/purchase";
import { computeRentalTaxCz } from "./countries/cz/taxes";
import { monthlyFixedOpexCz, monthlyGrossRentCz } from "./countries/cz/rental";

export function runSimulation(
  scenario: InvestmentScenario,
  market: MarketId = "it",
): AnalysisResult {
  if (market === "cz") return runSimulationCz(scenario);
  return runSimulationIt(scenario);
}

function runSimulationIt(scenario: InvestmentScenario): AnalysisResult {
...
function runSimulationCz(scenario: InvestmentScenario): AnalysisResult {
  const purchaseCosts = computePurchaseCostsCz(scenario);
  const initialCash = purchaseCosts.total_initial_cash;
  ...
  let cumulative = -initialCash;
  let propertyValue = scenario.property.purchase_price;
  const fixedOpex = monthlyFixedOpexCz(scenario);
  ...
  for (let monthIdx = 0; monthIdx < totalMonths; monthIdx++) {
    ...
    const mtg = monthIdx < mortgageSchedule.length ? mortgageSchedule[monthIdx] : null;
    const grossRent = monthlyGrossRentCz(scenario);
    const platformFee = monthlyPlatformFee(grossRent, scenario);
    const cleaningFee = monthlyCleaningFee(scenario) + monthlyTurnoverFee(scenario);
    const agencyFee = monthlyAgencyFee(scenario);
    const rentalTax = computeRentalTaxCz(grossRent, scenario);
